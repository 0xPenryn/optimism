...
func PopulateSuperchainState(env *Env, opcmAddr common.Address, superchainConfigProxy common.Address) (*addresses.SuperchainContracts, *addresses.SuperchainRoles, error) {
	...
	if env.UseForge {
		if env.ForgeClient == nil {
			return nil, nil, fmt.Errorf("Forge client is nil but UseForge is enabled")
		}
		if env.Context == nil {
			env.Context = context.Background()
		}
		forgeCaller := opcm.NewReadSuperchainDeploymentForgeCaller(env.ForgeClient)
		forgeOpts := []string{
			"--rpc-url", env.L1RPCUrl,
		}
		out, _, err = forgeCaller(env.Context, input, forgeOpts...)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to run ReadSuperchainDeployment with Forge: %w", err)
		}
	} else {
