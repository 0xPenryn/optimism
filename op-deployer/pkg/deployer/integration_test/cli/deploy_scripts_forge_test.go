...
func TestDeployScriptsForge(t *testing.T) {
	...
	t.Run("deploy altda with forge", func(t *testing.T) {
		...
		// Deploy AltDA using Forge
		deployAltDA := opcm.NewDeployAltDAForgeCaller(forgeClient)
		output, _, err := deployAltDA(
			context.Background(),
			opcm.DeployAltDAInput{
			...
			"--rpc-url", runner.GetL1RPC(),
			"--broadcast",
			"--private-key", runner.GetPrivateKey(),
		)
		require.NoError(t, err)
	...
	t.Run("deploy alphabet vm with forge", func(t *testing.T) {
		...
		// Deploy AlphabetVM using Forge
		deployAlphabetVM := opcm.NewDeployAlphabetVMForgeCaller(forgeClient)
		output, _, err := deployAlphabetVM(
			context.Background(),
			opcm.DeployAlphabetVMInput{
	...
	t.Run("deploy mips with forge", func(t *testing.T) {
		...
		// Deploy MIPS using Forge
		deployMIPS := opcm.NewDeployMIPSForgeCaller(forgeClient)
		output, _, err := deployMIPS(
			context.Background(),
	...
	t.Run("deploy dispute game with forge", func(t *testing.T) {
		...
		// Deploy DisputeGame using Forge
		deployDisputeGame := opcm.NewDeployDisputeGameForgeCaller(forgeClient)
		output, _, err := deployDisputeGame(
			context.Background(),
	...
	t.Run("read superchain deployment with forge", func(t *testing.T) {
		...
		// Read superchain deployment using Forge
		readSuperchainDeployment := opcm.NewReadSuperchainDeploymentForgeCaller(forgeClient)
		output, _, err := readSuperchainDeployment(
			context.Background(),
